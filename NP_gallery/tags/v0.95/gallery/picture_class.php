...
class PICTURE {
	...
	function setqueryold($query ='') {
		$pid = requestVar('id');
		global $manager;
		$defaultorder = $NPG_CONF['defaultorder'];
		$sorting = array('title'=>'title',
						'desc'=>'description',
						'owner'=>'ownername',
						'date'=>'modified',
						'titlea'=>'title',
						...
						'filenamea'=>'filename',
						'filename'=>'filename');
		$sortingascdesc = array('title'=>'ASC',
						'desc'=>'ASC',
						'owner'=>'ASC',
						'date'=>'DESC',
						'titlea'=>'DESC',
						...
						'filenamea'=>'ASC',
						'filename'=>'DESC');
		if ($sortingascdesc[$sort]=='ASC'){$oppositeorder = 'DESC';}
		if ($sortingascdesc[$sort]=='DESC'){$oppositeorder = 'ASC';};
	...
	function setquery($query ='') {
		...
		$order = array('title'=>' ASC',
						...
						'filenamea'=>' ASC',
						'filename'=>' DESC');
		if ($order[$sort]==' ASC'){$oppositeorder = ' DESC';}
		if ($order[$sort]==' DESC'){$oppositeorder = ' ASC';};
		...
		while ($row = mysql_fetch_object($result)){
			$i = $i + 1;
			if($row->pictureid==$this->id){$offset = $i;}
