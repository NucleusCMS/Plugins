...
function sqlite_modifyQueryForUserFunc(&$query,$strpositions,$pattern=null,$replacement=null){
	...
	foreach ($pattern as $key=>$pat) {
		...
		for ($i=1;$i<=$num;$i++) {
			...
			$temp=preg_replace_callback($pat,'sqlite_modifyQueryForUserFuncCallBack', $query, $i);
			$ok=true;
		...
		$query=preg_replace_callback($pat,'sqlite_modifyQueryForUserFuncCallBack', $query);
	}
}
...
sqlite_createQueryFunction('ASCII', 'ord');

sqlite_createQueryFunction('BIN', 'decbin');

sqlite_createQueryFunction( 'BIT_LENGTH', 'sqlite_userfunc_BIT_LENGTH');
function sqlite_userfunc_BIT_LENGTH($p1){
	return strlen($p1)*8;
}

sqlite_createQueryFunction( 'CHAR', 'sqlite_userfunc_CHAR');
...
sqlite_createQueryFunction( 'CHAR_LENGTH', 'mb_strlen');

sqlite_createQueryFunction( 'CONCAT_WS', 'sqlite_userfunc_CONCAT_WS');
...
sqlite_createQueryFunction( 'CONV', 'sqlite_userfunc_CONV');
...
sqlite_createQueryFunction( 'ELT', 'sqlite_userfunc_ELT');
...
sqlite_createQueryFunction( 'EXPORT_SET', 'sqlite_userfunc_EXPORT_SET');
...
sqlite_createQueryFunction( 'FIELD', 'sqlite_userfunc_FIELD');
...
sqlite_createQueryFunction( 'HEX', 'sqlite_userfunc_HEX');
...
sqlite_createQueryFunction( 'INSERT', 'sqlite_userfunc_INSERT');
...
sqlite_createQueryFunction( 'LOCATE', 'sqlite_userfunc_LOCATE');
...
sqlite_createQueryFunction( 'INSTR', 'sqlite_userfunc_INSTR');
function sqlite_userfunc_INSTR($p1,$p2){
	return sqlite_userfunc_LOCATE($p2,$p1);
}

sqlite_createQueryFunction( 'LCASE', 'sqlite_userfunc_LOWER');
sqlite_createQueryFunction( 'LOWER', 'sqlite_userfunc_LOWER');
function sqlite_userfunc_LOWER($p1){
	if (function_exists('mb_strtolower')) return mb_strtolower($p1);
	return strtolower($p1);
}

sqlite_createQueryFunction( 'LEFT', 'sqlite_userfunc_LEFT');
function sqlite_userfunc_LEFT($p1,$p2){
	if (function_exists('mb_substr')) return mb_substr($p1,0,$p2);
	return substr($p1,0,$p2);
}

sqlite_createQueryFunction( 'LENGTH', 'strlen');
sqlite_createQueryFunction( 'OCTET_LENGTH', 'strlen');

sqlite_createQueryFunction( 'LOAD_FILE', 'sqlite_userfunc_LOAD_FILE');
...
sqlite_createQueryFunction( 'LPAD', 'sqlite_userfunc_LPAD');
function sqlite_userfunc_LPAD($p1,$p2,$p3){
	return substr(str_repeat($p3,$p2/strlen($p3)).$p1,0-$p2);
}

sqlite_createQueryFunction( 'LTRIM', 'ltrim');

sqlite_createQueryFunction( 'MAKE_SET', 'sqlite_userfunc_MAKE_SET');
...
sqlite_createQueryFunction( 'MID', 'sqlite_userfunc_SUBSTRING');
sqlite_createQueryFunction( 'SUBSTRING', 'sqlite_userfunc_SUBSTRING');
...
sqlite_createQueryFunction( 'OCT', 'sqlite_userfunc_OCT');
function sqlite_userfunc_OCT($p1){
	if ($p1===null) return null;
	return sqlite_userfunc_CONV($p1,10,8);
}

sqlite_createQueryFunction( 'ORD', 'sqlite_userfunc_ORD');
...
sqlite_createQueryFunction( 'QUOTE', 'sqlite_userfunc_QUOTE');
function sqlite_userfunc_QUOTE($p1){
	if ($p1===null) return 'NULL';
	return str_replace(array("'","\\","\x1A"),array("\\'","\\\\","\\z"),$p1);
}

sqlite_createQueryFunction( 'REPEAT', 'str_repeat');

sqlite_createQueryFunction( 'REVERSE', 'sqlite_userfunc_REVERSE');
...
sqlite_createQueryFunction( 'RIGHT', 'sqlite_userfunc_RIGHT');
function sqlite_userfunc_RIGHT($p1){
	if (function_exists('mb_substr')) return mb_substr($p1,0-$p2);
	return substr($p1,0-$p2);
}

sqlite_createQueryFunction( 'RPAD', 'sqlite_userfunc_RPAD');
function sqlite_userfunc_RPAD($p1,$p2,$p3){
	return substr($p1.str_repeat($p3,$p2/strlen($p3)),0,$p2);
}

sqlite_createQueryFunction( 'RTRIM', 'rtrim');
sqlite_createQueryFunction( 'SOUNDEX', 'soundex');

sqlite_createQueryFunction( 'SPACE', 'sqlite_userfunc_SPACE');
function sqlite_userfunc_SPACE($p1){
	return str_repeat(' ',$p1);
}

sqlite_createQueryFunction( 'SUBSTRING_INDEX', 'sqlite_userfunc_SUBSTRING_INDEX');
...
sqlite_createQueryFunction( 'TRIM', 'sqlite_userfunc_TRIM');
...
sqlite_createQueryFunction( 'UCASE', 'sqlite_userfunc_UPPER');
sqlite_createQueryFunction( 'UPPER', 'sqlite_userfunc_UPPER');
function sqlite_userfunc_UPPER($p1){
	if (function_exists('mb_strtoupper')) return mb_strtoupper($p1);
	return strtoupper($p1);
}

sqlite_createQueryFunction( 'ACOS', 'acos');
sqlite_createQueryFunction( 'ASIN', 'asin');

sqlite_createQueryFunction( 'ATAN', 'sqlite_userfunc_ATAN');
...
sqlite_createQueryFunction( 'CEIL', 'ceil');
sqlite_createQueryFunction( 'CEILING', 'ceil');
sqlite_createQueryFunction( 'COS', 'cos');

sqlite_createQueryFunction( 'COT', 'sqlite_userfunc_COT');
function sqlite_userfunc_COT($p1){
	return 1/tan($p1);
}

sqlite_createQueryFunction( 'CRC32', 'crc32');

sqlite_createQueryFunction( 'DEGREES', 'sqlite_userfunc_DEGREES');
function sqlite_userfunc_DEGREES($p1){
	return ($p1/pi)*180;
}

sqlite_createQueryFunction( 'EXP', 'exp');
sqlite_createQueryFunction( 'FLOOR', 'floor');
sqlite_createQueryFunction( 'GREATEST', 'max');
sqlite_createQueryFunction( 'MAX', 'max');
sqlite_createQueryFunction( 'LEAST', 'min');
sqlite_createQueryFunction( 'MIN', 'min');
sqlite_createQueryFunction( 'ln', 'log');

sqlite_createQueryFunction( 'log', 'sqlite_userfunc_LOG');
function sqlite_userfunc_LOG($p1,$p2=null){
	if ($p2) return log($p1)/log($p2);
	return log($p1);
}

sqlite_createQueryFunction( 'log2', 'sqlite_userfunc_LOG2');
function sqlite_userfunc_LOG2($p1){
	return log($p1)/log(2);
}

sqlite_createQueryFunction( 'log10', 'log10');

sqlite_createQueryFunction( 'MOD', 'sqlite_userfunc_MOD');
function sqlite_userfunc_MOD($p1,$p2){
	return $p1 % $p2;
}

sqlite_createQueryFunction( 'PI', 'sqlite_userfunc_PI');
function sqlite_userfunc_PI(){
	return pi;
}

sqlite_createQueryFunction( 'POW', 'sqlite_userfunc_POW');
sqlite_createQueryFunction( 'POWER', 'sqlite_userfunc_POW');
function sqlite_userfunc_POW($p1,$p2){
	return pow($p1,$p2);
}

sqlite_createQueryFunction( 'RADIANS', 'sqlite_userfunc_RADIANS');
function sqlite_userfunc_RADIANS($p1){
	return ($p1/180)*pi;
}

sqlite_createQueryFunction( 'RAND', 'sqlite_userfunc_RAND');
function sqlite_userfunc_RAND($p1=null){
	if ($p1) srand($p1);
	return rand(0,1073741823)/1073741824;
}

sqlite_createQueryFunction( 'SIGN', 'sqlite_userfunc_SIGN');
...
sqlite_createQueryFunction( 'SIN', 'sin');
sqlite_createQueryFunction( 'SQRT', 'sqrt');
sqlite_createQueryFunction( 'TAN', 'tan');

sqlite_createQueryFunction( 'TRUNCATE', 'sqlite_userfunc_TRUNCATE');
function sqlite_userfunc_TRUNCATE($p1,$p2){
	$p2=pow(10,$p2);
	return ((int)($p1*$p2))/$p2;
}

sqlite_createQueryFunction( 'FORMAT', 'sqlite_userfunc_FORMAT');
function sqlite_userfunc_FORMAT($p1,$p2){
	return number_format($p1, $p2, '.', ',');
}

sqlite_createQueryFunction( 'INET_ATON', 'sqlite_userfunc_INET_ATON');
function sqlite_userfunc_INET_ATON($p1){
	$a=explode('.',$p1);
	return (($a[0]*256+$a[1])*256+$a[2])*256+$a[3];
}

sqlite_createQueryFunction( 'INET_NTOA', 'sqlite_userfunc_INET_NTOA');
...
sqlite_createQueryFunction( 'MD5', 'md5');


sqlite_createQueryFunction( 'CURDATE', 'sqlite_userfunc_CURDATE');
sqlite_createQueryFunction( 'CURRENT_DATE', 'sqlite_userfunc_CURDATE');
function sqlite_userfunc_CURDATE(){
	return date('Y-m-d');
}

sqlite_createQueryFunction( 'CURTIME', 'sqlite_userfunc_CURTIME');
sqlite_createQueryFunction( 'CURRENT_TIME', 'sqlite_userfunc_CURTIME');
function sqlite_userfunc_CURTIME(){
	return date('H:i:s');
}

sqlite_createQueryFunction( 'CURRENT_TIMESTAMP', 'sqlite_userfunc_NOW');
sqlite_createQueryFunction( 'LOCALTIME', 'sqlite_userfunc_NOW');
sqlite_createQueryFunction( 'LOCALTIMESTAMP', 'sqlite_userfunc_NOW');
sqlite_createQueryFunction( 'SYSDATE', 'sqlite_userfunc_NOW');
function sqlite_userfunc_NOW(){
	return date('Y-m-d H:i:s');
}

sqlite_createQueryFunction( 'DATE_FORMAT', 'sqlite_userfunc_DATE_FORMAT');
sqlite_createQueryFunction( 'TIME_FORMAT', 'sqlite_userfunc_DATE_FORMAT');
function sqlite_userfunc_DATE_FORMAT($p1,$p2){
	$t=sqlite_resolvedatetime($p1,$yr,$mt,$dy,$hr,$mn,$sc);
	$func='if ($matches=="%") return "%";';
	$func='return date($matches,'.$t.');';
	return preg_replace_callback ('/%([%a-zA-Z])/',create_function('$matches',$func), $p2);
}

sqlite_createQueryFunction( 'DAYNAME', 'sqlite_userfunc_DAYNAME');
function sqlite_userfunc_DAYNAME($p1){
	$t=sqlite_resolvedatetime($p1,$yr,$mt,$dy,$hr,$mn,$sc);
	return date('l',$t);
}

sqlite_createQueryFunction( 'DAYOFWEEK', 'sqlite_userfunc_DAYOFWEEK');
function sqlite_userfunc_DAYOFWEEK($p1){
	$t=sqlite_resolvedatetime($p1,$yr,$mt,$dy,$hr,$mn,$sc);
	return date('w',$t)+1;
}

sqlite_createQueryFunction( 'DAYOFYEAR', 'sqlite_userfunc_DAYOFYEAR');
function sqlite_userfunc_DAYOFYEAR($p1){
	$t=sqlite_resolvedatetime($p1,$yr,$mt,$dy,$hr,$mn,$sc);
	return date('z',$t);
}

sqlite_createQueryFunction( 'EXTRACT', 'sqlite_userfunc_EXTRACT');
function sqlite_userfunc_EXTRACT($p1,$p2){
	$t=sqlite_resolvedatetime($p2,$yr,$mt,$dy,$hr,$mn,$sc);
	switch(strtoupper($p1)) {
		case'SECOND': // SECONDS 
			return $sc;
		case'MINUTE': // MINUTES 
...
sqlite_createQueryFunction( 'FROM_DAYS', 'sqlite_userfunc_FROM_DAYS');
function sqlite_userfunc_FROM_DAYS($p1){
	return date('Y-m-d',($p1-719528)*86400);
}

sqlite_createQueryFunction( 'FROM_UNIXTIME', 'sqlite_userfunc_FROM_UNIXTIME');
...
sqlite_createQueryFunction( 'HOUR', 'sqlite_userfunc_HOUR');
function sqlite_userfunc_HOUR($p1){
	sqlite_resolvedatetime($p1,$yr,$mt,$dy,$hr,$mn,$sc);
	return $hr;
}

sqlite_createQueryFunction( 'MINUTE', 'sqlite_userfunc_MINUTE');
function sqlite_userfunc_MINUTE($p1){
	sqlite_resolvedatetime($p1,$yr,$mt,$dy,$hr,$mn,$sc);
	return $mn;
}

sqlite_createQueryFunction( 'MONTHNAME', 'sqlite_userfunc_MONTHNAME');
...
sqlite_createQueryFunction( 'PERIOD_ADD', 'sqlite_userfunc_PERIOD_ADD');
...
sqlite_createQueryFunction( 'PERIOD_DIFF', 'sqlite_userfunc_PERIOD_DIFF');
...
sqlite_createQueryFunction( 'QUARTER', 'sqlite_userfunc_QUARTER');
...
sqlite_createQueryFunction( 'SECOND', 'sqlite_userfunc_SECOND');
function sqlite_userfunc_SECOND($p1){
	sqlite_resolvedatetime($p1,$yr,$mt,$dy,$hr,$mn,$sc);
	return $sc;
}

sqlite_createQueryFunction( 'SEC_TO_TIME', 'sqlite_userfunc_SEC_TO_TIME');
function sqlite_userfunc_SEC_TO_TIME($p1){
	return date('H:i:s',$p1);
}

sqlite_createQueryFunction( 'WEEK', 'sqlite_userfunc_WEEK');
function sqlite_userfunc_WEEK($p1){
	$t=sqlite_resolvedatetime($p1,$yr,$mt,$dy,$hr,$mn,$sc);
	return date('W',$t);
}

sqlite_createQueryFunction( 'WEEKDAY', 'sqlite_userfunc_WEEKDAY');
...
sqlite_createQueryFunction( 'YEAR', 'sqlite_userfunc_YEAR');
function sqlite_userfunc_YEAR($p1){
	$t=sqlite_resolvedatetime($p1,$yr,$mt,$dy,$hr,$mn,$sc);
	return date('Y',$t);
}

sqlite_createQueryFunction( 'YEARWEEK', 'sqlite_userfunc_YEARWEEK');
...
sqlite_createQueryFunction( 'FIND_IN_SET', 'sqlite_userfunc_FIND_IN_SET');
...
sqlite_createQueryFunction( 'IF', 'sqlite_userfunc_IF');
function sqlite_userfunc_IF($p1,$p2,$p3){
	if ((int)$p1) return $p2;
	return $p3;
}

sqlite_createQueryFunction( 'IFNULL', 'sqlite_userfunc_IFNULL');
function sqlite_userfunc_IFNULL($p1,$p2){
	if ($p1!=null) return $p1;
	return $p2;
}

sqlite_createQueryFunction( 'NULLIF', 'sqlite_userfunc_NULLIF');
...
sqlite_createQueryFunction( 'match_against', 'sqlite_userfunc_match_against');
...
sqlite_createQueryFunction( 'replace', 'sqlite_userfunc_replace');
function sqlite_userfunc_replace($p1,$p2,$p3){
	return str_replace($p3,$p1,$p2);
}

sqlite_createQueryFunction( 'UNIX_TIMESTAMP', 'sqlite_userfunc_UNIX_TIMESTAMP');
...
sqlite_createQueryFunction( 'REGEXP', 'sqlite_userfunc_REGEXP');
function sqlite_userfunc_REGEXP($p1,$p2){
	return preg_match ("/$p2/",$p1);
}

sqlite_createQueryFunction( 'DAYOFMONTH', 'sqlite_userfunc_DAYOFMONTH');
function sqlite_userfunc_DAYOFMONTH($p1){
	sqlite_resolvedatetime($p1,$yr,$mt,$dy,$hr,$mn,$sc);
	return $dy;
}

sqlite_createQueryFunction( 'MONTH', 'sqlite_userfunc_MONTH');
...
sqlite_createQueryFunction( 'sqlite_table_structure', 'sqlite_userfunc_sqlite_table_structure');
